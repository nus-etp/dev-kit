...
Optionally set `RESEND_API_KEY` + `RESEND_FROM` to send login OTP emails via
[Resend](https://resend.com). If not set, OTP emails are logged to the console
instead (fine for local dev and no-auth prototypes). See
[docs/EMAIL_SETUP.md](docs/EMAIL_SETUP.md) for the self-serve setup.
...
| `RESEND_API_KEY`  | An API key to send login OTP email via Resend (only if your app has login). See docs/EMAIL_SETUP.md                       | re_xxxxxxxxxxxxxxxxxxxxxxxxxx                                                |
| `RESEND_FROM`     | The verified sender address for OTP email (only if your app has login)                                                   | My App &lt;noreply@my-app.nusx.edu.sg&gt;                                    |
...
> **Deploying many apps under one shared Vercel account?** See
> [SETUP.md](SETUP.md) for the self-serve, deploy-hook-based flow (flat cost,
> any committer can deploy).
